...
def _derive_business_family(product: Dict[str, Any]) -> str:
...
def _derive_factory(spec: Dict[str, Any]) -> str:
...
def _derive_org(spec: Dict[str, Any]) -> str:
    family = str(spec.get("family", ""))
    if family in {"DP", "DA", "PCO", "DC", "DI", "DS"}:
        return "ASSEMBLY"
    if family in {"FCB", "BM", "PC", "WB"}:
...
def _build_base_row(date: str, spec: Dict[str, Any], product: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "FAMILY": _derive_business_family(product),
        "FACTORY": _derive_factory(spec),
        "ORG": _derive_org(spec),
        "라인": spec["라인"],
    }
...
def _apply_signal_overrides(dataset_key: str, row: Dict[str, Any]) -> Dict[str, Any]:
    process_name = str(row.get("OPER_NAME", ""))
    mode = str(row.get("MODE", ""))
    den = str(row.get("DEN", ""))

    # Build a few stable signal patterns so question-based validation is easier.
...
def get_production_data(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for spec, product in _iter_valid_process_product_pairs():
        ...
        row["production"] = qty
        row = _apply_signal_overrides("production", row)
        rows.append(row)
...
def get_target_data(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for spec, product in _iter_valid_process_product_pairs():
        ...
        row = _build_base_row(date, spec, product)
        row["target"] = target
        row = _apply_signal_overrides("target", row)
        rows.append(row)
    rows = _apply_common_filters(rows, params)
...
def get_yield_data(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for spec, product in _iter_valid_process_product_pairs():
        ...
        row = _apply_signal_overrides("yield", row)
        rows.append(row)
    rows = _apply_common_filters(rows, params)
...
def get_hold_lot_data(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for index, (spec, product) in enumerate(_iter_valid_process_product_pairs(), start=1):
        ...
        row = _apply_signal_overrides("hold", row)
        rows.append(row)
    rows = _apply_common_filters(rows, params)
