...
from .contracts import DatasetProfile, PreprocessPlan
...
DIMENSION_ALIAS_MAP = {
    ...
    "PKG1": {"PKG1", "pkg1"},
    "PKG2": {"PKG2", "pkg2"},
    "TSV_DIE_TYP": {"TSV_DIE_TYP", "tsv_die_typ", "tsv", "hbm", "3ds"},
...
def dataset_profile(data: List[Dict[str, Any]]) -> DatasetProfile:
    return {"columns": extract_columns(data), "row_count": len(data), "sample_rows": list(data[:3])}
...
def _resolve_requested_column(token: str, columns: List[str]) -> str | None:
    ...
    ignored_tokens = {normalize_text(item) for item in IGNORED_DIMENSION_TOKENS}
...
def find_requested_dimensions(query_text: str, columns: List[str]) -> List[str]:
    ...
    for pattern in [r"([\w/\-가-힣]+)\s*별", r"([\w/\-가-힣]+)\s*기준"]:
        for raw_token in re.findall(pattern, query_text):
            resolved_name = _resolve_requested_column(str(raw_token).strip(), columns)
            if resolved_name:
...
def minimal_fallback_plan(query_text: str, data: List[Dict[str, Any]]) -> PreprocessPlan:
    ...
    return {
        ...
        "warnings": ["LLM plan generation failed, so a minimal fallback sort was used."],
        "source": "fallback",
        "code": f"result = df.sort_values(by={metric_column!r}, ascending={str(sort_order == 'asc')}).head({top_n})",
    }
