...
def _resolve_model_name(task: str) -> str:
    ...
    If the user does not set separate env vars, both task groups safely fall
    back to the same model. That means this feature adds flexibility without
    making local setup harder.
    """
...
- 존재하지 않는 데이터나 컬럼은 지어내지 않는다.
- 답변은 항상 현재 결과 테이블 기준으로 설명한다.
"""
