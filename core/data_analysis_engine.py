...
from .domain_registry import match_registered_analysis_rules
...
def _find_semantic_retry_reason(query_text: str, columns: List[str], code: str) -> str:
    ...
    if "hold_reason" in available and ("대표 hold 사유" in query or "최다 hold 사유" in query):
    ...
    if ("없는" in query or "목록" in query or "list" in lower_query) and not any(
        token in code_text for token in ["isna(", "isnull(", "notna(", "notnull("]
    ):
        return "The previous code did not apply missing-value filtering even though the user asked for rows that exist in one dataset but not another."
...
def _pick_ratio_operands(rule: Dict[str, Any], columns: List[str]) -> tuple[str | None, str | None]:
    ...
    numerator = (
        role_map.get("numerator")
        or role_map.get("left")
        or role_map.get("base")
        or (ordered_columns[0] if ordered_columns else None)
    )
    denominator = (
...
def _build_domain_rule_fallback_plan(query_text: str, data: List[Dict[str, Any]]) -> Dict[str, Any] | None:
    ...
    if group_columns:
        code_lines.append(
            f"grouped = df.groupby({group_columns!r}, as_index=False).agg(\n" + ",\n".join(agg_lines) + "\n)"
        )
    else:
        overall_items = ", ".join(
            f"{helper_names[column_name]!r}: [df[{column_name!r}].sum()]"
    ...
    for item in ratio_rules:
        ...
        code_lines.append(
            f"grouped[{item['output_column']!r}] = grouped[{numerator_name!r}] / grouped[{denominator_name!r}]"
        )
    ...
    return {
        ...
        "warnings": ["LLM 분석 대신 등록된 도메인 계산 규칙으로 안전하게 계산했습니다."],
        "code": "\n".join(code_lines),
...
def _success_result(
    ...
    return {
        ...
        "data": result_rows,
        "summary": f"현재 데이터 분석 결과: {len(result_rows)}건",
        "analysis_plan": plan,
