...
def build_dataset_specific_hints(data: List[Dict[str, Any]], query_text: str) -> str:
    ...
    if "yield_rate" in columns and "공정군" in columns and "OPER_NAME" in columns and ("최저 수율 공정" in query or "가장 낮은 수율 공정" in query):
        hints.append(
    ...
    if "production" in columns and "target" in columns and ("달성율" in query or "달성률" in query or "목표 대비" in query):
        hints.append(
            "- `목표 대비 생산 달성율` 예시: grouped = df.groupby('OPER_NAME', as_index=False).agg(production=('production', 'sum'), target=('target', 'sum')); grouped['달성율'] = grouped['production'] / grouped['target']; result = grouped"
        )
...
- If a matched custom rule includes `source_columns`, use those dataset/column roles as the primary hint
  for how to build the derived metric or status flag.
- If a matched custom rule includes `condition` or `decision_rule`, treat them as direct instructions for
  creating a derived column, boolean flag, or status label in pandas code.
- If a matched custom rule includes `output_column`, prefer using that name for the derived column.
- If a matched custom rule includes `default_group_by` and the user does not clearly ask for another grain,
  use that grouping level.
...
Good status-flag example:
- If a matched custom rule says that rows with status HOLD or REWORK are abnormal,
  a good code shape is:
  result = df.copy()
  result['hold_abnormal_flag'] = result['상태'].isin(['HOLD', 'REWORK']).map({{True: '이상', False: '정상'}})
