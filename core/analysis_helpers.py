...
DIMENSION_ALIAS_MAP = {
    ...
    "라인": {"라인", "line"},
    "MODE": {"MODE", "mode", "모드", "제품"},
    "DEN": {"DEN", "den", "density", "용량"},
    "TECH": {"TECH", "tech", "기술"},
    "LEAD": {"LEAD", "lead"},
    "MCP_NO": {"MCP_NO", "mcp", "mcp_no"},
    "OPER_NUM": {"OPER_NUM", "oper_num", "oper", "공정번호", "operation"},
    # Reserve `pkg1` / `pkg2` for the literal PKG1 / PKG2 columns.
    # This avoids ambiguous mapping between legacy PKG1/PKG2 and
    # canonical PKG_TYPE1/PKG_TYPE2 names.
...
def _resolve_requested_column(token: str, columns: List[str]) -> str | None:
    ...
    # Exact column match comes first so a user can safely ask for FAMILY
    # without being redirected to another alias such as 공정군.
...
def find_requested_dimensions(query_text: str, columns: List[str]) -> List[str]:
    ...
    # Exact current column names win first.
    for column in columns:
...
def minimal_fallback_plan(query_text: str, data: List[Dict[str, Any]]) -> PreprocessPlan:
    ...
    return {
        ...
        "warnings": ["LLM 코드 생성이 실패해 최소 fallback 로직을 사용했습니다."],
        "source": "fallback",
        "code": (
